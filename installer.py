...
GITHUB = "Valen7440/eventball/contents/"
PACKAGE_PATH = "ballsdex/packages/eventball_cog"
PACKAGE_FILES = [
    "__init__.py",
    "card.py",
    "cog.py", 
]
TORTOISE_MODELS = ["eventball_models.py"]
DJANGO_APP_FILES = {
    "eventball_app": [
        "admin.py", 
        "apps.py", 
        ...
        "migrations/0002_eventball_created_at_eventball_emoji_id.py"
    ],
}

os.makedirs("ballsdex/packages/eventball_cog", exist_ok=True)
os.makedirs("admin_panel/eventball_app", exist_ok=True)
...
async def install_package_files():
    ...
    async with aiohttp.ClientSession() as session:
        for index, file in enumerate(files):
            data = await fetch_github_file(session, f"https://api.github.com/repos/{GITHUB}/eventball_package/cog/{file}")
...
async def install_tortoise_models():
    ...
    async with aiohttp.ClientSession() as session:
        for index, file in enumerate(TORTOISE_MODELS):
            data = await fetch_github_file(session, f"https://api.github.com/repos/{GITHUB}/eventball_package/{file}")
...
async def install_django_app_files(app_name: str):
    ...
    async with aiohttp.ClientSession() as session:
        for index, file in enumerate(files):
            data = await fetch_github_file(session, f"https://api.github.com/repos/{GITHUB}/eventball_package/{app_name}/{file}")
...
await install_django_app_files("eventball_app")
